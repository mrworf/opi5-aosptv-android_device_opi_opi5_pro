...
::android::status_t StreamPrimary::init(DriverCallbackInterface* callback) {
    if (!mIsInput) {
        // Output streams default to the real ALSA card so audio reaches hardware
        // before setConnectedDevices is called. setConnectedDevices can still
        // switch to stub for telephony/bus devices.
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (ReadFileToString(cardPath, &cardName)) {
            if (deviceName == "jack" && (cardName.starts_with("Headphones")
                ...
                return cardAndDeviceId;
            } else if (deviceName == "hdmi0" && (cardName.starts_with("hdmi0")
                    || cardName.starts_with("rockchip-hdmi0")
                    || cardName.starts_with("rockchip_hdmi0")
                    || cardName.starts_with("vc4hdmi0"))) {
                LOG(INFO) << "Using PCM card " << i << " for HDMI0 audio";
                cardAndDeviceId.first = i;
                return cardAndDeviceId;
            } else if (deviceName == "hdmi1" && (cardName.starts_with("hdmi1")
                    || cardName.starts_with("rockchip-hdmi1")
                    || cardName.starts_with("rockchip_hdmi1")
                    || cardName.starts_with("vc4hdmi1"))) {
                LOG(INFO) << "Using PCM card " << i << " for HDMI1 audio";
                cardAndDeviceId.first = i;
                return cardAndDeviceId;
            } else if (deviceName == "dac" && !cardName.starts_with("Headphones")
                    && !cardName.starts_with("es8388")
                    && !cardName.starts_with("hdmi")
                    && !cardName.starts_with("vc4hdmi")
                    && !cardName.starts_with("rockchip-hdmi")
                    && !cardName.starts_with("rockchip_hdmi")) {
