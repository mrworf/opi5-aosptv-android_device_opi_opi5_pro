...
static bool is_hdmi(char *hdmi_device_name, int card) {
    ...
    if (!strcmp(hdmi_device, "hdmi0") || !strcmp(hdmi_device, "hdmi1")) {
        // Use hw: plugin with the actual card number for direct PCM access.
        // Previously used "default:CARD=vc4hdmi0" which is RPi-specific (vc4-hdmi driver).
        // RK3588 uses simple-card named "hdmi0" with no vc4 prefix, so that path fails.
        sprintf(hdmi_device_name, "hw:%d,0", card);
        return true;
    }
        ...
        const alsa_device_profile * profile)
{
    ...
    proxy->hdmi = is_hdmi(proxy->hdmi_device_name, profile->card);
